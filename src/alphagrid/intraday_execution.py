...
def order_for(signal,symbol,account):
    ...
    qty=int(min(equity*D('.14')/price,max(D(0),cash-D('100'))/price,equity*D('.0075')/(price-stop)))
    ...
    def try_candidate(self,symbol):
        ...
        with process_lock(self.root/'state/execution.lock',timeout=20):
            ...
            if len(positions)>=12:
                ...
                rotation=None
                order=order_for(signal,symbol,account)
                if int(order['qty'])<1:
                    return None
                ...
                if not self.ledger.reserve(payload,'Operator-authorized experimental IEX continuation; 3R bracket, 45-minute/15:50 ET exit. No established profitability.'):
                    return None
                fresh_watchdog(self.ledger)
        ...
        with process_lock(Path(root)/'state/worker.lock'):
            ...
            try:
                while True:
                    ...
                    result.update(execution_enabled=result['status']!='halted',paper=True,reported_at=utcnow().isoformat())
