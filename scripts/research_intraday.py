...
for s in symbols:
    ...
    path=root/'data'/f'intraday_history_{args.feed}_{s}.json';path.write_text(json.dumps(raw))
...
result={'strategy':'intraday_iex_news_5m_v1' if args.feed=='iex' else 'intraday_continuation_5m_v1',
        'feed':args.feed,'news_used_in_entry_rule':False,'source_universe_observed_at':scan['at'],'symbols':symbols,
...
(root/'reports'/f'intraday_backtest_{args.feed}.json').write_text(json.dumps(result,indent=2,allow_nan=False))
